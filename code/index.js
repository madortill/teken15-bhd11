    ...
    switch (objCurrentMed.type) {
        case 'text':
            for(let i = 0; i < objCurrentMed.content.length; i++){
                ...
                parah.innerHTML = objCurrentMed.content[i];
                document.querySelector(`.content`).append(parah); 
            }
            break;
        case 'listDots': case 'listNumbers':
            let p = El("h4", {cls : "h-list"}, objCurrentMed.discription);
                p.innerHTML = objCurrentMed.discription;
            ...
            if(objCurrentMed.num !== 1) {
                if(objCurrentMed.type === "listNumbers"){
                    for (let i = 2; i <= objCurrentMed.num; i++) {
                        let p2 = El("h4", {cls : "h-list"}, objCurrentMed[`discription${i}`]);
                        p2.innerHTML = objCurrentMed[`discription${i}`];
                        document.querySelector(`.content`).append(p2)
                            if (objCurrentMed[`content${i}`] !== undefined) {
                                let parah2 = El("p", {cls : "listItem2"},);
                                parah2.innerHTML = objCurrentMed[`content${i}`];
                                document.querySelector(`.content`).append(parah2); 
                            }
                    }
                } else {
                    for (let i = 2; i <= objCurrentMed.num; i++) {
                        p = El("h4", {cls : "h-list"}, objCurrentMed[`discription${i}`]);
                        // p = El("h4", {cls : "h-list"}, objCurrentMed.discription);
                        p.innerHTML = objCurrentMed[`discription${i}`];
                        document.querySelector(`.content`).append(p);
                        if (objCurrentMed[`content${i}`] !== undefined) {
                            for (let j = 0; j < objCurrentMed[`content${i}`].length; j++) {
                                parah = El("li", {cls : "listItem"}, objCurrentMed[`content${i}`][j]);
                                document.querySelector(`.content`).append(parah); 
                            }
                        } 
                    }
...
const creatMedShelfs = (event) => {
        ...
        if(key !== "pic" && key !== "color"){
            ...
            for(let subSubject of Object.keys(objInfo[strCurrentSubject][key])) {
                if(subSubject !== "discription"){
                    ...
                    let discription = El("div", {});
                    discription.innerHTML = objInfo[strCurrentSubject][key].discription;
                    document.querySelector(`.${strCurrentSubject}ShelfContainer`).append(discription);                    
                }
            }
            index++;
        }
    ...
    bSearchScreen = false;
    document.querySelector('.medTypeSymbol').classList.add("hidden");
    // myInterval = setInterval(color, 120);
}
...
// const color = () => {
//     nColor++;
//     document.querySelector(`.test`).style.filter = `hue-rotate(${nColor}deg)`;
//     document.querySelector(`.topButton`).style.filter = `hue-rotate(${nColor}deg)`;
//     document.querySelector(`.homeButton`).style.filter = `hue-rotate(${nColor}deg)`;
//     document.querySelector(`.searchBoxHolder`).style.filter = `hue-rotate(${nColor}deg)`;
// }